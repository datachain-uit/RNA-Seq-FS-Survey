...
from src.features.svm_rfe_selector import SVMRFESelector
...
def main():
    ...
    parser.add_argument("--outdir", default="outputs/svm_rfe/")
    ...
    selector = SVMRFESelector(k=args.k)
    ...
    np.save(f"{args.outdir}/svmrfe_mask.npy", mask)
    ...
    df.to_csv(f"{args.outdir}/metrics_svmrfe.csv", index=False)

    print(df)
    print("SVM-RFE FS Completed.")
